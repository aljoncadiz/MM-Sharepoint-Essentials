...
import * as strings from 'ThirdWpKnockoutWebPartStrings';
import ThirdWpKnockoutViewModel, { IThirdWpKnockoutBindingContext } from './ThirdWpKnockoutViewModel';

let _instance: number = 0;

export interface IThirdWpKnockoutWebPartProps {
  description: string;
}

export default class ThirdWpKnockoutWebPart extends BaseClientSideWebPart<IThirdWpKnockoutWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: IThirdWpKnockoutBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: ThirdWpKnockoutViewModel,
        template: require('./ThirdWpKnockout.template.html'),
        synchronous: false
      }
    );
  }

...
import * as strings from 'KnockoutFrameworkWebPartStrings';
import KnockoutFrameworkViewModel, { IKnockoutFrameworkBindingContext } from './KnockoutFrameworkViewModel';

let _instance: number = 0;

export interface IKnockoutFrameworkWebPartProps {
  description: string;
}

export default class KnockoutFrameworkWebPart extends BaseClientSideWebPart<IKnockoutFrameworkWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: IKnockoutFrameworkBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: KnockoutFrameworkViewModel,
        template: require('./KnockoutFramework.template.html'),
        synchronous: false
      }
    );
  }
